...
struct State {
    ...
    double h; // Ước lượng chi phí đến trạng thái đích
    double f; // Tổng chi phí g + h

    // So sánh để dùng trong hàng đợi ưu tiên
    bool operator<(const State& other) const {
        return f > other.f; // f nhỏ hơn sẽ được ưu tiên
    }
};

// Hàm AKT (thuật toán tìm kiếm trên cây với kiến thức)
vector<int> AKT(
    ...
    function<double(int)> h      // Hàm ước lượng h'
) {
    ...
    open.push({ start, 0, h(start), h(start) });
    g_map[start] = 0;
    parent[start] = -1; // Trạng thái bắt đầu không có cha

    while (!open.empty()) {
        // a. Chọn trạng thái Tmax có giá trị f nhỏ nhất và loại khỏi OPEN
        State Tmax = open.top();
        open.pop();
        ...
        for (const auto& neighbor : neighbors) {
            ...
            if (parent[Tmax.id] != Tk) {
                ...
                if (g_map.find(Tk) == g_map.end() || g_new <= g_map[Tk]) {
                    g_map[Tk] = g_new;
                    double h_new = h(Tk);  // Tính h'(Tk)
                    double f_new = g_new + h_new;  // f(Tk) = g(Tk) + h'(Tk)

                    // Cập nhật trạng thái cha của Tk là Tmax.id
                    parent[Tk] = Tmax.id;

                    // Thêm Tk vào OPEN
                    open.push({ Tk, g_new, h_new, f_new });
                }
            }
        }
    }
...
void docDuLieuTuFile(const string& filename, int& num_nodes, vector<double>& heuristic, unordered_map<int, vector<pair<int, double>>>& graph, int& start, int& goal) {
